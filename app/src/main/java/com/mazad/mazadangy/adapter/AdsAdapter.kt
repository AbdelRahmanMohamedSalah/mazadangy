...
class AdsAdapter() : RecyclerView.Adapter<AdsAdapter.VH>() {
    ...
    lateinit var arryImage: ArrayList<String>
    ...
    constructor(context: Context, adsList: ArrayList<AdsModel>, fromActivity: String) : this() {
        ...
        if (fromActivity.equals("posts")) {
            ...
            databaseRefrance = database.getReference("posts")
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.startPriceTv.text = adsList[position].start_price+" جنيه "
        holder.endPriceTv.text = adsList[position].end_price+" جنيه "
        ...
        printDifferenceDateForHours(
            adsList[position].end_ads,
            adsList[position].imge,
            holder.countDownTimerTextView
        )
        // holder.nameUserTv.text = "" + name
        var imgeList: List<String> = adsList[position].imge
        arryImage = adsList[position].imge as ArrayList<String>
        Picasso.get().load(imgeList[0]).into(holder.imgeAdIv)
    ...
    fun printDifferenceDateForHours(endDateDay: String, imageList: List<String>, dateTv: TextView) {
...
//                })
                ...
                for (x in 0 until imageList.size step 1) {
                    storageReference = firebaseStorage.getReferenceFromUrl(imageList[x])
                    storageReference.delete()
                    println(" the image number = " + x + " = " + imageList[x])
